...
public class NotificationController : ControllerBase
{
    ...
    /// <summary>Send bulk notifications — Admin broadcast</summary>
    [HttpPost("bulk")]
    ...
    public async Task<IActionResult> SendEmail([FromBody] SendEmailDto dto)
    {
        ...
        if (!result) return BadRequest(new { message = "Failed to send email." });
    ...
    public async Task<IActionResult> SendSms([FromBody] SendSmsDto dto)
    {
        ...
        if (!result) return BadRequest(new { message = "Failed to send SMS." });
        return Ok(new { message = "SMS sent successfully." });
    }

    /// <summary>Get ALL notifications — Admin use</summary>
    [HttpGet]
    ...
    /// <summary>Get notifications for a recipient</summary>
